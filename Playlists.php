...
<div id="home" style="background-color: cyan">
  ...
  $query="SELECT pname,pid from playlist where userid='$userid'";
  $result=mysqli_query($db,$query);
  $arr=array();
  $brr=array();
    while($row=mysqli_fetch_array($result)){
        array_push($arr,$row['pname']);
        array_push($brr,$row['pid']);
    }
   ?>
<script>
    ...
    var table = '<div class="grid-item">'+'<div class="grid-container"><a href="createplaylist.php"><img src="img/CreatePlaylist.png" style="width:5pc;height: 5pc;"><p>CreatePlaylist</p></a></div>'+ '</div>';
    var rows = len;
    for( r = 0 ; r < rows ; r++)
    {
        table += '<div class="grid-item"><a href="';
        table += 'ShowsongsOfPlaylist.php';
        table += '">';
        table += '<img src="img/playlist.png" style="width:5pc;height: 5pc;">';
        table += '<button name="btnn">';
        table += complex[r];
        table += '</button></a>';
        table +='</div>';
        <?php
            session_start();
            echo $_GET['r'];
            $t="<script>document.write(r);</script>";
            $_SESSION['user']=$brr[$_GET['r']];
            echo $_SESSION['user'];
            echo $t;
        ?>
    } 
