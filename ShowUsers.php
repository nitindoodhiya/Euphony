...
<script>
    ...
    <?php foreach($userids as $key => $val){ ?>
        userIds.push('<?php echo $val; ?>');
    <?php } ?>
    ...
    {
        table += '<div class="grid-item" style="padding:3pc">'   ;
        table += '<a href="OthersProfile.php">';
        table += '<img src="img/icon.png" style="width:4pc;height: 4pc;"><form action="othersProfile.php" method="POST">'
        table +='<input type = "hidden" name = "Name" value = userIds />';    
        table += '<button type="submit">'+ complex[r] + '</button></form>';
        table += '</a>';
        table +='</div>';
    } 
